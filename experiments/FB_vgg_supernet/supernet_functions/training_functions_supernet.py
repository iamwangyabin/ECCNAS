import torch
import time
from general_functions.utils import AverageMeter
    ...
    def _training_step(self, model, loader, optimizer, epoch, info_for_logger=""):
        ...
        for step, (inputs, points, targets, st_sizes) in enumerate(loader):
            ...
            if step % self.print_freq == 0:
                self.logger.info(
                    'Epoch {} Step {} Train, Loss: {:.2f}/({:.2f}), MSE: {:.2f} MAE: {:.2f}, Cost {:.1f} sec'
                    .format(epoch, step, epoch_loss.get_avg(), loss.item(), np.sqrt(epoch_mse.get_avg()),
                            epoch_mae.get_avg(), time.time() - start_time))

        self.writer.add_scalar('train_vs_val/' + "train" + '_loss' + info_for_logger, epoch_loss.get_avg(), epoch)
        self.writer.add_scalar('train_vs_val/' + "train" + '_mse' + info_for_logger, np.sqrt(epoch_mse.get_avg()),
                               epoch)
