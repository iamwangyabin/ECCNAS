...
def train_supernet():
    ...
    #### Model
    model = FBNet_Stochastic_SuperNet().cuda()
    ...
    w_optimizer = torch.optim.Adam(params=filter(lambda p: p.requires_grad, params_except_thetas),
    ...
    # w_scheduler = None
    last_epoch = -1
    w_scheduler = torch.optim.lr_scheduler.StepLR(w_optimizer, step_size=20, gamma=0.9, last_epoch=last_epoch)
