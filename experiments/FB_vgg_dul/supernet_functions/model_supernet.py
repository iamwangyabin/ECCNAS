...
class FBNet_Stochastic_SuperNet(nn.Module):
    def __init__(self, branch1, branch2, branch3):
        ...
        self.features_train = make_layers(cfg['E'])
    ...
    def forward(self, x, temperature):
        y1 = self.features(x)
        # y2 = self.features_train(x)
        y1 = F.upsample_bilinear(y1, scale_factor=2)
        # y2 = F.upsample_bilinear(y2, scale_factor=2)
        # import pdb;pdb.set_trace()
        y = torch.cat([y1, y1],dim=1)
        y = self.reg_layer(y)
        return torch.abs(y)
